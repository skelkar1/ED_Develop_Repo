...
//
//  Created by Sarika Kelkar on 10/05/22.
//
...
//                XCTAssertEqual(item, expectedItem(at:index), "Item not matching at \(index)")
//            }
            
        case let .failure(error):
            XCTFail("Expected successful feed result, got \(error) instead")
        default:
            XCTFail("Expected successful feed result, got no response instead")
        }
    }
